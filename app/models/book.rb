	...
	def import(driver: nil)
		...
		begin
			...
			if title_read
				...
				self.genres ||= driver.find_elements(css: 'a.bookPageGenreLink').map(&:text).join('|')
				...
				#self.publisher ||= driver.find_element(xpath: "//div[@class='row' and contains(text(),'Published')]").text.strip.split('by').last.strip
				self.genres ||= driver.find_elements(css: 'span.BookPageMetadataSection__genreButton').map(&:text).join('|')
